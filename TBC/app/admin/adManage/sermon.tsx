...
const Sermon = (props) => {
  ...
  const [isStarred, setIsStarred] = useState(props.isFeatured === '1');
  ...
  const toggleStar = async (itemId) => {
    ...
    await updateFeaturedStatus(itemId, !videoStates[itemId]); 
  };

  async function updateFeaturedStatus(docId, isFeatured) {
    const docRef = doc(db, "sermon", docId);
    await updateDoc(docRef, { isFeatured: isFeatured ? '1' : '0' });
  }

  useEffect(() => {
    const fetchVideos = async () => {
      try {
        const videosRef = collection(db, 'sermon');
        ...
        const initialVideoStates = fetchedVideos.reduce((acc, video) => {
          acc[video.id] = false;
          return acc;
        }, {});
        ...
        renderItem={({ item }) => (
          <View style={styles.videoCard}>
            ...
            <View style={styles.videoInfo}>
              ...
              <Ionicons
                name={item.isFeatured === '1' ? 'star-sharp' : 'star-outline'}
                size={20}
                color={item.isFeatured === '1' ? Colors.primary : isDarkMode ? 'white' : 'black'}
                onPress={() => toggleStar(item.id)}
                style={{ alignSelf: 'flex-end', bottom: 40, paddingRight: 10 }}
              />
            </View>
          </View>
        )}
